class PendingsController < ApplicationController
  ...
  def index
    ...
    @contents = Content.where(group_id: @group.id,status: 2).order("endDate DESC").page(params[:page]).per(5)  
    @contentss = Content.where(group_id: @group.id,status: 1).order("endDate DESC").page(params[:page]).per(5)  
